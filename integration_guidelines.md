...
### Security
#### Exchange Authorization Tokens & HTTPS

IOP follows the security standard mechanisms of OCPI. At the connection, GIREVE provides new operator connecting with temporary Token(s) to register as CPO and/or eMSP. The Token should be used when the connection is initiated by the operator.
...
IOP also uses customer IP filtering as an additional security measure. This ensures that only trusted sources can access the API, reinforcing security and protecting against unauthorized access attempts.

**<ins>Calling partners must provide a list of authorised IP addresses before connecting to the platform.</ins>**

#### SSL client certificate security

To enhance security, Gireve configures authentication via SSL client certificates in both directions. 

However, **<ins>only the client certificate used by the partner to send requests to IOP, which is generated by Gireve, is mandatory.</ins>** The client certificate used by IOP to send requests to the partner, provided by the partner, is optional.
